...
float CalcShadowFactor(in vec3 LightDirection, in float ViewDistance, in float bias)                                                  
{
	...
	float Factor = 0.0f;	
	for (int x = 0; x < 20; ++x) {	
		const vec4 FinalCoord1		= vec4(LightDirection + sampleOffsetDirections[x] * diskRadius, (shadowBuffers[shadowIndexes[BufferIndex]].Shadow_Spot/6));
		const vec4 FinalCoord2		= vec4(LightDirection + sampleOffsetDirections[x] * diskRadius, (shadowBuffers[shadowIndexes[BufferIndex]].Shadow_Spot/6)+1);
...
{		
	...
	const vec3 LightDirection 		= (lightBuffers[lightIndexes[BufferIndex]].LightPosition.xyz - data.World_Pos.xyz);
	...
	// Direct Light	
	vec3 Fs;
	const vec3 Radiance 			= ( (lightBuffers[lightIndexes[BufferIndex]].LightColor.xyz * lightBuffers[lightIndexes[BufferIndex]].LightIntensity) * Attenuation ) * ShadowFactor;
