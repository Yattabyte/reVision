...
void Shadow_FBO::initialize(EnginePackage * enginePackage)
{
	if (!m_Initialized) {
		...
		for (int x = 0; x < SHADOW_MAX; ++x) {
			...
			glTextureImage3DEXT(m_shadow_worldnormal[x], GL_TEXTURE_2D_ARRAY, 0, GL_RGB8, m_size[x].x, m_size[x].y, 1, 0, GL_RGB, GL_FLOAT, NULL);
			...
			glNamedFramebufferTexture(m_shadow_fbo[x], GL_COLOR_ATTACHMENT0, m_shadow_worldnormal[x], 0);

			// Create the Radiant Flux buffer
			glTextureImage3DEXT(m_shadow_radiantflux[x], GL_TEXTURE_2D_ARRAY, 0, GL_RGB8, m_size[x].x, m_size[x].y, 1, 0, GL_RGB, GL_FLOAT, NULL);
			...
			glNamedFramebufferTexture(m_shadow_fbo[x], GL_COLOR_ATTACHMENT1, m_shadow_radiantflux[x], 0);

			GLenum Buffers[] = {GL_COLOR_ATTACHMENT0,
								GL_COLOR_ATTACHMENT1};
			glNamedFramebufferDrawBuffers(m_shadow_fbo[x], 2, Buffers);
...
void Shadow_FBO::registerShadowCaster(const int & shadow_type, int & array_spot)
{
	...
	glTextureImage3DEXT(m_shadow_worldnormal[shadow_type], GL_TEXTURE_2D_ARRAY, 0, GL_RGB8, m_size[shadow_type].x, m_size[shadow_type].y, m_shadow_count[shadow_type], 0, GL_RGB, GL_FLOAT, NULL);
	glTextureImage3DEXT(m_shadow_radiantflux[shadow_type], GL_TEXTURE_2D_ARRAY, 0, GL_RGB8, m_size[shadow_type].x, m_size[shadow_type].y, m_shadow_count[shadow_type], 0, GL_RGB, GL_FLOAT, NULL);
}
...
{
	glBindTextureUnit(ShaderTextureUnit, m_shadow_depth[ShadowSpot]);
...
void Shadow_FBO::setSize(const unsigned int & shadow_type, const float & size)
{
	...
	glTextureImage3DEXT(m_shadow_worldnormal[shadow_type], GL_TEXTURE_2D_ARRAY, 0, GL_RGB8, m_size[shadow_type].x, m_size[shadow_type].y, m_shadow_count[shadow_type], 0, GL_RGB, GL_FLOAT, NULL);
	glNamedFramebufferTexture(m_shadow_fbo[shadow_type], GL_COLOR_ATTACHMENT0, m_shadow_worldnormal[shadow_type], 0);

	glTextureImage3DEXT(m_shadow_radiantflux[shadow_type], GL_TEXTURE_2D_ARRAY, 0, GL_RGB8, m_size[shadow_type].x, m_size[shadow_type].y, m_shadow_count[shadow_type], 0, GL_RGB, GL_FLOAT, NULL);
	glNamedFramebufferTexture(m_shadow_fbo[shadow_type], GL_COLOR_ATTACHMENT1, m_shadow_radiantflux[shadow_type], 0);
}
