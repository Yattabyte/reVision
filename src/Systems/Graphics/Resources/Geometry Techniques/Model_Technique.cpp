...
{
	const size_t size = camera.getVisibilityToken().specificSize(Model_Animated_C::GetName());
...
void Model_Technique::writeCameraBuffers(Camera & camera, const unsigned int & instanceCount)
{
	...
	const size_t size = vis_token.specificSize(Model_Animated_C::GetName());
	if (size) {
		...
		for each (const auto &component in vis_token.getTypeList<Model_Animated_C>()) {
