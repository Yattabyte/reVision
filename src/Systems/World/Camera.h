...
public:
	...
	Camera(const glm::vec3 & position = glm::vec3(0.0F), const glm::vec2 & size = glm::vec2(1.0f), const float & near_plane = 0.5f, const float & far_plane = 10.0f, const float & horizontal_FOV = 90.0f);
	/** Copy constructor. */
	 ...
	 * @brief				this makes the camera visible to all shaders at index '1' */
	void bind() const;
	/** Retrieve the camera's position in world space.
	 * @return				the camera's world position */
	const glm::vec3 getPosition() const { std::shared_lock<std::shared_mutex> rguard(data_mutex);  return m_cameraBuffer.EyePosition; }
	/** Change the camera's position in space.
	 * @param	p			the new position value to use */
	void setPosition(const glm::vec3 & p) { std::unique_lock<std::shared_mutex> wguard(data_mutex); m_cameraBuffer.EyePosition = p; };
	/** Change the camera's orientation in space.
	 * @param	q			new quaternion value to use */
	void setOrientation(const glm::quat & q) { std::unique_lock<std::shared_mutex> wguard(data_mutex); m_orientation = q; };
	...
	void setDimensions(const glm::vec2 & d) { std::unique_lock<std::shared_mutex> wguard(data_mutex); m_cameraBuffer.Dimensions = d; };
	...
	void setNearPlane(const float & n) { std::unique_lock<std::shared_mutex> wguard(data_mutex); m_cameraBuffer.NearPlane = n; };
	...
	void setFarPlane(const float & f) { std::unique_lock<std::shared_mutex> wguard(data_mutex); m_cameraBuffer.FarPlane = f; };
	/** Change the camera's horizontal FOV.  
	 * @param	fov			the new horizontal FOV to use */
	void setHorizontalFOV(const float & fov) { std::unique_lock<std::shared_mutex> wguard(data_mutex); m_cameraBuffer.FOV = fov; };
	/** Change the camera's gamma.
	 * @param	gamma		the new gamma value */
	void setGamma(const float & gamma) { std::unique_lock<std::shared_mutex> wguard(data_mutex); m_cameraBuffer.Gamma = gamma; };
	/** Change the perspective and viewing matrices.
	 * @param	pMatrix		new perspective matrix
	 * @param	vMatrix		new viewing matrix */
	void setMatrices(const glm::mat4 & pMatrix, const glm::mat4 &vMatrix);
	/** Return a copy of this camera's visibility token.
	 * @return				reference to the visibility token */
	const Visibility_Token getVisibilityToken() const { std::shared_lock<std::shared_mutex> rguard(data_mutex);  return m_vistoken; };
	...
	const Camera_Buffer getCameraBuffer() const { std::shared_lock<std::shared_mutex> rguard(data_mutex);  return m_cameraBuffer; };
	...
	/** Retrieve the std::mutex associated with data changes. 
	 * @return				the data std::mutex */
	std::shared_mutex & getDataMutex() const { return data_mutex; };
