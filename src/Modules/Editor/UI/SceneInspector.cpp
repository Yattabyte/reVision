			...
			std::function<void(const EntityHandle&)> displayEntity = [&](const EntityHandle& entityHandle) {
				if (const auto entity = ecsWorld.getEntity(entityHandle)) {
					...
					if (entity_or_components_pass_filter) {
						...
						if (std::find(selectedEntities.cbegin(), selectedEntities.cend(),
						...
						const auto tryRightClickElement = [&] {
							if (ImGui::BeginPopupContextItem("Entity Controls")) {
								...
								for (int x = 0; x < nameLength && x < 256; ++x)
								...
								if (const auto selectionSize = selectedEntities.size()) {
									if (selectionSize >= 2ull) {
										const auto text =
											"Join to \"" +
											ecsWorld.getEntity(selectedEntities[0])->m_name + "\"";
						...
						if (ImGui::TreeNodeEx(entity.get(), node_flags, "%s", entityName.c_str())) {
							...
							tryDragElement();
							for (const auto& subEntityHandle :
								ecsWorld.getEntityHandles(entityHandle))
							...
							if (ImGui::BeginPopupContextItem("Add New Component", 0)) {
								...
								constexpr const char* items[] = {
									Transform_Component::Name,		PlayerSpawn_Component::Name,
									Player3D_Component::Name,		Camera_Component::Name,
									BoundingSphere_Component::Name, BoundingBox_Component::Name,
									Prop_Component::Name,			Skeleton_Component::Name,
									Shadow_Component::Name,			Light_Component::Name,
									Reflector_Component::Name,		Collider_Component::Name,
								};
