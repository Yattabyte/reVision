...
void Translation_Gizmo::render(const float& deltaTime)
{
	...
	if (m_model->existsYet() && m_colorPalette->existsYet() && m_gizmoShader->existsYet() && m_editor->getSelection().size()) {
		...
		const auto dir = glm::normalize(camPos - m_position);
		m_directions = glm::vec3(
			dir.x > 0 ? 1 : -1,
			dir.y > 0 ? 1 : -1,
			dir.z > 0 ? 1 : -1
		);
		...
		auto mMatrix = glm::translate(glm::mat4(1.0f), m_position) * glm::scale(glm::mat4(1.0f), m_directions * glm::vec3(glm::distance(m_position, m_engine->getModule_Graphics().getClientCamera()->get()->EyePosition) * 0.02f));
...
void Translation_Gizmo::setPosition(const glm::vec3& position)
{
	m_position = position;
}
...
bool Translation_Gizmo::rayCastMouse(const float& deltaTime)
{
	...
	// Flip the model's axes based on which side of it were on
	const auto dir = glm::normalize(ray_origin - m_position);
	...
	if (m_selectedAxes == NONE) {
		...
		const auto scalingFactor = direction * glm::distance(m_position, m_engine->getModule_Graphics().getClientCamera()->get()->EyePosition) * 0.02f;
		const auto mMatrix = glm::translate(glm::mat4(1.0f), m_position);
		...
		for (int x = 0; x < 3; ++x) {
			float intDistance;
			if (!glm::intersectRayPlane(ray_origin, ray_world, m_position, plane_normals[x], intDistance))
				glm::intersectRayPlane(ray_origin, ray_world, m_position, -plane_normals[x], intDistance);
		...
		m_axisDelta = m_startingOffset - m_position;
	...
	// An axis is now selected, perform dragging operation
		...
		m_position = endingOffset - m_axisDelta;
		m_editor->moveSelection(m_position);
	}
