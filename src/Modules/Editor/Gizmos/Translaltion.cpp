...
void Translation_Gizmo::frameTick(const float& deltaTime)
{
	checkMouseInput(deltaTime);
	render(deltaTime);
}

void Translation_Gizmo::checkMouseInput(const float& deltaTime)
{
...
glm::vec3 Translation_Gizmo::getPosition() const
{
	return m_position;
}

void Translation_Gizmo::rayCastMouse(const float& deltaTime)
{
	m_engine->getModule_World().updateSystem(m_pickerSystem, deltaTime);
	const auto& [entity, position] = ((MousePicker_System*)m_pickerSystem)->getTranslation();

	// Set translation to all tools that need it	
	if (ImGui::GetIO().KeyCtrl)
		m_editor->toggleAddToTranslation(entity);
	else
		if (entity == NULL_ENTITY_HANDLE) {
			m_editor->setTranslation({});
			m_editor->setGizmoPosition(position);
		}
		else
			m_editor->setTranslation({ entity });
}