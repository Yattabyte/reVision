...
public:
	...
	inline virtual void updateComponents(const float & deltaTime, const std::vector< std::vector<BaseECSComponent*> > & components) override {
		...
		std::tuple<EntityHandle, float, glm::vec3> closest = { NULL_ENTITY_HANDLE, FLT_MAX, glm::vec3(0.0f) };
		for each (const auto & componentParam in components) {
			...
			if (collider) {
				...
				if (auto distance = hit_sphere(transformComponent->m_transform.m_position + bSphere->m_positionOffset, bSphere->m_radius); distance >= 0.0f)
					if (distance < std::get<1>(closest)) {
						checkSuccessfull = true;
						closest = { transformComponent->entity, distance, transformComponent->m_transform.m_position };
					}

			}
			else if (!checkSuccessfull) {
				// Create scaling factor to keep all origins same screen size
				const auto radius = glm::distance(transformComponent->m_transform.m_position, m_engine->getModule_Graphics().getClientCamera()->get()->EyePosition) * 0.033f;

				// Check if the distance is closer than the last entity found, so we can find the 'best' selection
				if (auto distance = hit_sphere(transformComponent->m_transform.m_position, radius); distance >= 0.0f)
					if (distance < std::get<1>(closest)) {
						checkSuccessfull = true;
						closest = { transformComponent->entity, distance, transformComponent->m_transform.m_position };
					}
	...
	// Public Methods
	/***/
	std::pair<EntityHandle, glm::vec3> getSelection() {
