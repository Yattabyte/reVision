...
class Scrollbar_H : public UI_Decorator
{
public:
	// Interaction enums
	enum interact {
	...
	Scrollbar_H(Engine * engine, const std::shared_ptr<UI_Element> & component) : UI_Decorator(component) {
		...
		panel->setMinScale(glm::vec2(25.0f, 12.5f));
		panel->setMaxScale(glm::vec2(m_scale.x - 25.0f - 25.0f, 12.5f));
	...
	virtual void update() override {
		...
		updateElements();
		UI_Element::update();
		m_component->setPosition(glm::vec2(0, 12.5f));
		m_component->setScale(glm::vec2(m_scale.x, m_scale.y - 12.5f));
	}
			...
			if (consumed) {
				if (m_children.size() == 3) {
					if (std::dynamic_pointer_cast<Button>(m_children[2])->getPressed()) {
						setLinear(subEvent.m_xPos / (m_scale.x - 25.0f - 25.0f));
						updateElements();
					}
				}
			}
...
protected:
	...
	void updateElements() {
		if (m_children.size() == 3) {
			// Buttons
			m_children[0]->setPosition(glm::vec2(-m_scale + 12.5f));
			m_children[1]->setPosition(glm::vec2(m_scale.x - 12.5f, -m_scale.y + 12.5f));

			// Panel
			m_children[2]->setMaxScale(glm::vec2(m_scale.x - 25.0f - 25.0f, 12.5f));
			m_children[2]->setPosition(glm::vec2(m_linear * (m_scale.x - 25.0f - 25.0f), -getScale().y + 12.5f));
		}
	}
