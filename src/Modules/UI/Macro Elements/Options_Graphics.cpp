	...
	Options_Pane(engine)
{
	...
	addOption(engine, element_material_list, 1.0f, "Texture Quality:", "Adjusts the resolution of in-game geometry textures.", (int)SideList::Interact::on_index_changed, [&, element_material_list]() noexcept { setTextureResolution(element_material_list->getIndex()); });
	...
	addOption(engine, element_shadow_list, 1.0f, "Shadow Quality:", "Adjusts the resolution of all dynamic light shadows textures.", (int)SideList::Interact::on_index_changed, [&, element_shadow_list]() noexcept { setShadowSize(element_shadow_list->getIndex()); });
	...
	addOption(engine, element_env_list, 1.0f, "Reflection Quality:", "Adjusts the resolution of all environment map textures.", (int)SideList::Interact::on_index_changed, [&, element_env_list]() noexcept { setReflectionSize(element_env_list->getIndex()); });
	...
	addOption(engine, element_bounce_list, 1.0f, "Light Bounce Quality:", "Adjusts the resolution of the real-time GI simulation.", (int)SideList::Interact::on_index_changed, [&, element_bounce_list]() noexcept { setBounceQuality(element_bounce_list->getIndex()); });
	...
	addOption(engine, maxShadow_slider, 0.75f, "Max Concurrent Shadows:", "Set the maximum number of shadows updated per frame.", (int)Slider::Interact::on_value_change, [&, maxShadow_slider]() noexcept {
	...
	addOption(engine, maxReflection_slider, 0.75f, "Max Concurrent Reflections:", "Set the maximum number of reflections updated per frame.", (int)Slider::Interact::on_value_change, [&, maxReflection_slider]() noexcept {
	...
	addOption(engine, element_bloom, 0.5f, "Bloom:", "Turns the bloom effect on or off.", (int)Toggle::Interact::on_toggle, [&, element_bloom]() noexcept { setBloom(element_bloom->isToggled()); });
	...
	addOption(engine, element_ssao, 0.5f, "SSAO:", "Turns screen-space ambient occlusion effect on or off. Works with baked AO.", (int)Toggle::Interact::on_toggle, [&, element_ssao]() noexcept { setSSAO(element_ssao->isToggled()); });
	...
	addOption(engine, element_ssr, 0.5f, "SSR:", "Turns screen-space reflections on or off. Works with baked reflections.", (int)Toggle::Interact::on_toggle, [&, element_ssr]() noexcept { setSSR(element_ssr->isToggled()); });
	...
	addOption(engine, element_fxaa, 0.5f, "FXAA:", "Turns fast approximate anti-aliasing on or off.", (int)Toggle::Interact::on_toggle, [&, element_fxaa]() noexcept { setFXAA(element_fxaa->isToggled()); });
}
