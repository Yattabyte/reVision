...
public:
	...
	inline explicit Options_Video(Engine* engine) noexcept
		...
		addOption(engine, element_res, 1.0f, "Resolution:", "Changes the resolution the game renders at.", (int)SideList::Interact::on_index_changed, [&, element_res, engine]() {
			const auto& index = element_res->getIndex();
			engine->getPreferenceState().setValue(PreferenceState::Preference::C_WINDOW_WIDTH, m_resolutions[index].x);
			engine->getPreferenceState().setValue(PreferenceState::Preference::C_WINDOW_HEIGHT, m_resolutions[index].y);
			engine->getPreferenceState().setValue(PreferenceState::Preference::C_WINDOW_REFRESH_RATE, m_resolutions[index].z);
			});
		...
		addOption(engine, gamma_slider, 0.75f, "Gamma:", "Changes the gamma correction value used.", (int)Slider::Interact::on_value_change, [&, gamma_slider, engine]() {
			...
			engine->getPreferenceState().setValue(PreferenceState::Preference::C_GAMMA, round_value);
			gamma_slider->setText(out.str());
			});
		...
		addOption(engine, ddistance_slider, 0.75f, "Draw Distance:", "Changes how far geometry can be seen from.", (int)Slider::Interact::on_value_change, [&, ddistance_slider, engine]() {
			engine->getPreferenceState().setValue(PreferenceState::Preference::C_DRAW_DISTANCE, ddistance_slider->getValue());
			});
		...
		addOption(engine, fov_slider, 0.75f, "Field of view:", "Changes how wide of an angle the scene can be viewed from.", (int)Slider::Interact::on_value_change, [&, fov_slider, engine]() {
			// Get a round version of the input
			const int round_value = (int)std::round(fov_slider->getValue());
			// We store as a float, but we want to ensure round numbers
			engine->getPreferenceState().setValue(PreferenceState::Preference::C_FOV, float((int)std::round(fov_slider->getValue())));
			});
		...
		addOption(engine, element_sync, 0.5f, "VSync:", "Lock the game's frame-rate to the monitor's refresh rate.", (int)Toggle::Interact::on_toggle, [&, element_sync, engine]() {
			engine->getPreferenceState().setValue(PreferenceState::Preference::C_VSYNC, element_sync->getToggled() ? 1.0f : 0.0f);
			});
		...
		addOption(engine, element_fs, 0.5f, "Full-screen:", "Render the game full-screen instead of as a window.", (int)Toggle::Interact::on_toggle, [&, element_fs, engine]() {
			engine->getPreferenceState().setValue(PreferenceState::Preference::C_WINDOW_FULLSCREEN, element_fs->getToggled() ? 1.0f : 0.0f);
			});
	}
