...
public:
	...
	: m_engine(engine) {
		...
		if (Status != GL_FRAMEBUFFER_COMPLETE && Status != GL_NO_ERROR)
			m_engine->getMessageManager().error(MessageManager::FBO_INCOMPLETE, "FXAA Framebuffer", std::string(reinterpret_cast<char const *>(glewGetErrorString(Status))));
		if (!glIsTexture(m_textureID))
			m_engine->getMessageManager().error(MessageManager::TEXTURE_INCOMPLETE, "FXAA Texture");
	}
