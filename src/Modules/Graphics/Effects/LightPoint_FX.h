...
class LightPoint_Effect : public GFX_Core_Effect {
public:
	// Public (de)Constructors
	/** Virtual Destructor. */
	inline ~LightPoint_Effect() {
		// Update indicator
	...
	inline LightPoint_Effect(
		...
		preferences.getOrSetValue(PreferenceState::C_SHADOW_SIZE_POINT, m_renderState->m_shadowSize.x);
		preferences.addCallback(PreferenceState::C_SHADOW_SIZE_POINT, m_aliveIndicator, [&](const float &f) { m_renderState->m_shadowSize = glm::ivec2(std::max(1, (int)f)); });		
		...
		if (glCheckNamedFramebufferStatus(m_shadowFBO.m_fboID, GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
			m_engine->getManager_Messages().error("PointLight Shadowmap Framebuffer has encountered an error.");
	}
