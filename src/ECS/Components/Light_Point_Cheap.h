...
public:
	...
	static const char * GetName() { return "Light_Point_Cheap"; }
...
protected:
	...
	Light_Point_Cheap_C(Engine * engine, const glm::vec3 & color, const glm::vec2 &intensity_radius, const Transform & transform);
	/** Creates the component from an argument list.
	 * @param	argumentList	the argument list to construct from */
	static Light_Point_Cheap_C * Create(const ArgumentList & argumentList);
