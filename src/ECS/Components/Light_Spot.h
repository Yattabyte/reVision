...
protected:
	...
	Light_Spot_C(Engine * engine, const glm::vec3 & color, const glm::vec2 &intensity_radius, const float & cutoff, const Transform & transform);
	/** Creates the component from an argument list.
	 * @param	argumentList	the argument list to construct from */
	static Light_Spot_C * Create(const ArgumentList & argumentList);
