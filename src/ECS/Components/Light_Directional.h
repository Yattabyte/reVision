...
protected:
	...
	Light_Directional_C(Engine * engine, const glm::vec3 & color, const float & intensity, const Transform & transform);
	/** Creates the component from an argument list.
	 * @param	argumentList	the argument list to construct from */
	static Light_Directional_C * Create(const ArgumentList & argumentList);
