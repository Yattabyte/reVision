...
public: 
	...
	) : BaseECSSystem(), m_engine(engine), m_geometryFBO(geometryFBO), m_lightingFBO(lightingFBO), m_bounceFBO(bounceFBO), m_volumeRH(volumeRH) {
	...
	virtual void updateComponents(const float & deltaTime, const std::vector< std::vector<BaseECSComponent*> > & components) override {
		...
		const GLuint bounceInstanceCount = GLuint(shadowIndices.size() * m_volumeRH->m_resolution);
