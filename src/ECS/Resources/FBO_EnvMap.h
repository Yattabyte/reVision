...
struct FBO_EnvMap : FBO_Base {
	...
	FBO_EnvMap() {
		...
		resize(512, 512, 6);
		glNamedFramebufferTexture(m_fboID, GL_COLOR_ATTACHMENT0, m_texture, 0);
	...
	void resize(const GLuint & width = 1, const GLuint & height = 1, const GLuint & depth = 1) {
		...
		for (int x = 0; x < 6; ++x) {
			...
			glTextureImage3DEXT(m_texture, GL_TEXTURE_CUBE_MAP_ARRAY, x, GL_RGB8, size.x, size.y, depth, 0, GL_RGB, GL_FLOAT, NULL);
		}
	}
	void clear(const GLint & zOffset) {
		const glm::vec3 clear(0.0f);
		glClearTexSubImage(m_texture, 0, 0, 0, zOffset, m_size.x, m_size.y, 6, GL_RGB, GL_FLOAT, &clear);
	}
	virtual void clear() {
	...
	virtual void bindForReading(const GLuint & binding = 0) {
		glBindTextureUnit(binding, m_texture);
	}
