...
struct FBO_Geometry : FBO_Base {
	...
	FBO_Geometry() {
		...
		glCreateTextures(GL_TEXTURE_2D, 4, m_textures);
		resize();
		...
		glNamedFramebufferTexture(m_fboID, GL_DEPTH_STENCIL_ATTACHMENT, m_textures[3], 0);
	...
	virtual void resize(const GLuint & width = 1, const GLuint & height = 1, const GLuint & depth = 1) {
		...
		glTextureImage2DEXT(m_textures[0], GL_TEXTURE_2D, 0, GL_RGBA8, m_size.x, m_size.y, 0, GL_RGBA, GL_FLOAT, NULL);
		glTextureImage2DEXT(m_textures[1], GL_TEXTURE_2D, 0, GL_RGB16F, m_size.x, m_size.y, 0, GL_RGB, GL_FLOAT, NULL);
		glTextureImage2DEXT(m_textures[2], GL_TEXTURE_2D, 0, GL_RGBA8, m_size.x, m_size.y, 0, GL_RGBA, GL_FLOAT, NULL);
		glTextureImage2DEXT(m_textures[3], GL_TEXTURE_2D, 0, GL_DEPTH24_STENCIL8, m_size.x, m_size.y, 0, GL_DEPTH_STENCIL, GL_UNSIGNED_INT_24_8, NULL);
	}
	virtual void clear() {
	...
	virtual void bindForReading(const GLuint & binding = 0) {
		for (int x = 0; x < 4; ++x)
		glBindTextureUnit(x + binding, m_textures[x]);
	}
