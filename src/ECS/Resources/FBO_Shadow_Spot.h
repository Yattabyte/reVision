...
struct FBO_Shadow_Spot {
	...
	void clear(const GLint & zOffset) {
		...
		glClearTexSubImage(m_textures[0], 0, 0, 0, zOffset, m_size.x, m_size.y, 1, GL_RGB, GL_FLOAT, &clear);
		glClearTexSubImage(m_textures[1], 0, 0, 0, zOffset, m_size.x, m_size.y, 1, GL_RGB, GL_FLOAT, &clear);
		glClearTexSubImage(m_textures[2], 0, 0, 0, zOffset, m_size.x, m_size.y, 1, GL_DEPTH_COMPONENT, GL_FLOAT, &clearDepth);
	}
	void bindForWriting() {
