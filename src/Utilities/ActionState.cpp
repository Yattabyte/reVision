...
float& ActionState::operator[](const ActionState::Action& index) noexcept 
{
	return std::get<1>(m_keyStates[index]);
}

const float& ActionState::operator[](const ActionState::Action& index) const noexcept 
{
	return std::get<1>(m_keyStates.at(index));
}

ActionState::State ActionState::isAction(const ActionState::Action& actionEnum) noexcept 
{
