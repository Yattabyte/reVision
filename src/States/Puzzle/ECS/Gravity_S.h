				...
				for (unsigned int x = 0u; x < 6u; ++x) {
					...
					switch (dTile.dropState) {
						...
						case Board_Component::TileDropData::FALLING: {
							...
							board.m_data->tiles[(y * 6) + x].yOffset = 2.0f * ((dTile.time / (dTile.delta * Tile_DropDuration)) * dTile.delta);
							break;
						}
						// Find Bouncing Tiles
						case Board_Component::TileDropData::BOUNCING: {
							...
							board.m_data->tiles[(y * 6) + x].yOffset = 2.0f * (1.0f - (easeOutBounce(adjustedTime / bounceTime))) * bounceHeight;
							break;
						}
						// Find Stationary Tiles
						default: {
							// Reset the data just in case
							board.m_data->tiles[(y * 6) + x].yOffset = 0.0f;
