...
Shared_Collider::Shared_Collider(Engine * engine, const std::string & filename, const bool & threaded)
	: std::shared_ptr<Asset_Collider>(std::dynamic_pointer_cast<Asset_Collider>(engine->getAssetManager().shareAsset(typeid(Asset_Collider).name(), filename)))
{
	...
	if (!get()) {
		...
		engine->getAssetManager().submitNewAsset(typeid(Asset_Collider).name(), (*(std::shared_ptr<Asset>*)(this)), std::move(std::bind(&Asset_Collider::initialize, get(), engine, (DIRECTORY_COLLIDER + filename))), threaded);
	}
