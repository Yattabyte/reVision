...
Shared_Level::Shared_Level(Engine * engine, const std::string & filename, const bool & threaded)
	: std::shared_ptr<Asset_Level>(std::dynamic_pointer_cast<Asset_Level>(engine->getAssetManager().shareAsset(typeid(Asset_Level).name(), filename)))
{
	...
	if (!get()) {
		...
		engine->getAssetManager().submitNewAsset(typeid(Asset_Level).name(), (*(std::shared_ptr<Asset>*)(this)), std::move(std::bind(&Asset_Level::initialize, get(), engine, (DIRECTORY_LEVEL + filename))), threaded);
	}
